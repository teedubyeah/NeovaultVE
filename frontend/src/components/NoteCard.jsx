...
export default function NoteCard({ note, onClick, onPin, onArchive, onDelete, onShare }) {
    ...
    <div className={`note-card color-${note.color || 'default'}`} onClick={onClick}>
      ...
      <div className="note-footer">
        ...
        <div className="note-actions">
          {onShare && (
            <button className="btn-icon" title="Share" onClick={stop(onShare)}>
          ...
          <button className="btn-icon" title={note.is_pinned ? 'Unpin' : 'Pin'} onClick={stop(onPin)}>            <svg width="13" height="13" viewBox="0 0 24 24" fill={note.is_pinned ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
