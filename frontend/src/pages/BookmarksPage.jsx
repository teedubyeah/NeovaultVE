...
// ─── FolderTreeItem ──────────────────────────────────────────────────────────
function FolderTreeItem({ folder, selectedId, onSelect, onRename, onDelete, onNewChild, onDrop, depth, bookmarkCounts }) {
  ...
  return (
    <div>
      <div
        ...
        {(bookmarkCounts?.[folder.id] ?? 0) > 0 && (
          <span style={{fontSize:10,color:'var(--text3)',background:'var(--bg3)',padding:'1px 6px',borderRadius:8,flexShrink:0,marginRight:2}}>
            {bookmarkCounts[folder.id]}
          </span>
        )}
      ...
      {expanded && hasChildren && folder.children.map(c=>(
        <FolderTreeItem key={c.id} folder={c} selectedId={selectedId} depth={depth+1}
          onSelect={onSelect} onRename={onRename} onDelete={onDelete} onNewChild={onNewChild} onDrop={onDrop} bookmarkCounts={bookmarkCounts}/>
      ))}
    </div>
  )
}
...
// ─── BookmarkCard ────────────────────────────────────────────────────────────
function BookmarkCard({ bookmark, folderName, onEdit, onDelete, onToggleFav, onShare }) {
    ...
    <div draggable
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
        ...
        <div style={{display:'flex',gap:4}}>
          <button className="btn-icon" style={{padding:'3px 6px',fontSize:12}} title="Share" onClick={()=>onShare(bookmark)}>🔗</button>
...
export default function BookmarksPage() {
  ...
  // Count bookmarks per folder (direct only — not recursive)
  const bookmarkCounts = {}
  bookmarks.forEach(b => { if (b.folder_id) bookmarkCounts[b.folder_id] = (bookmarkCounts[b.folder_id]||0)+1 })
    ...
    <div className="app-layout" onMouseUp={onDragEnd}>
      ...
      <header className="topbar">
        ...
        <div style={{display:'flex',gap:4,background:'var(--bg3)',borderRadius:8,padding:3,border:'1px solid var(--border)'}}>
          ...
          <Link to="/shared" style={{padding:'5px 14px',borderRadius:6,fontSize:12,fontWeight:600,letterSpacing:'0.5px',textTransform:'uppercase',textDecoration:'none',color:'var(--text3)'}}>🔗 Shared</Link>
        </div>
                ...
                {display.map(b=>(
                  <div key={b.id} onDragStart={onDragStart} onDragEnd={onDragEnd}>
                    <BookmarkCard bookmark={b} folderName={b.folder_id?folderMap[b.folder_id]?.name:null}
                      onShare={b=>setShareModal({itemId:b.id,itemTitle:b.title||b.url})}
              ...
              <div style={{display:'flex',flexDirection:'column',gap:2}}>
                {display.map(b=>(
                  <div key={b.id} draggable
                    ...
                    <button className="btn-icon" style={{padding:'2px 5px',fontSize:12}} title="Share" onClick={()=>setShareModal({itemId:b.id,itemTitle:b.title||b.url})}>🔗</button>
      ...
      {shareModal&&<ShareModal itemType="bookmark" itemId={shareModal.itemId} itemTitle={shareModal.itemTitle} onClose={()=>setShareModal(null)}/> }
