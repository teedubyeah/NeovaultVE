...
// _sessionPassword lives in sessionStorage so it survives a page refresh
// within the same browser tab. It is cleared on logout or tab close.
// sessionStorage is tab-isolated and never sent to the server except
// as the X-Password header on authenticated requests.
const SESSION_PW_KEY = 'nve_pw';

let _sessionPassword = sessionStorage.getItem(SESSION_PW_KEY) || null;

export function setSessionPassword(p) {
  _sessionPassword = p;
  if (p) sessionStorage.setItem(SESSION_PW_KEY, p);
...
export const shareApi = {
  create:          (body)   => request('/share',                          { method: 'POST',   body }),
  sent:            ()       => request('/share/sent'),
  received:        ()       => request('/share/received'),
  preview:         (token)  => request(`/share/preview/${token}`),
  accept:          (token)  => request(`/share/accept/${token}`,          { method: 'POST' }),
  revoke:          (id)     => request(`/share/${id}`,                    { method: 'DELETE' }),
  sharedWithItem:  (itemId) => request(`/share/shared-with-item/${itemId}`),
};

export const smtpApi = {
  get:  ()     => request('/admin/smtp'),
  save: (body) => request('/admin/smtp',      { method: 'POST', body }),
  test: (body) => request('/admin/smtp/test', { method: 'POST', body }),
};
