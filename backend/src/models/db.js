...
function initDb() {
  ...
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS shared_items (
      ...
      item_type TEXT NOT NULL CHECK(item_type IN ('note','bookmark')),
    ...
    CREATE INDEX IF NOT EXISTS idx_shared_recipient ON shared_items(recipient_email);
    CREATE INDEX IF NOT EXISTS idx_shared_token ON shared_items(token);
  ...
  // Migrate: shared_items table (added in sharing update)
  try {
    database.exec(`
      CREATE TABLE IF NOT EXISTS shared_items (
        id TEXT PRIMARY KEY,
  ...
  // SMTP settings table (admin-configurable, single row)
  database.exec(`
